...
namespace ShaderGraphGenerator.Editor
{
    ...
    public static class ShaderGenerationPipeline
    {
        ...
        {
            // 1. Write HLSL file
            string hlslPath = MaterialPreviewHelper.CreateHLSLFile(
            ...
            if (MaterialPreviewHelper.HasShaderCompileErrors(mat.shader))
        ...
        {
            string firstPrompt = ShaderPromptBuilder.BuildLLMPrompt(userPrompt, settings.UseTransparency);
            ...
            for (int iteration = 1; iteration <= MaxIterations; iteration++)
            {
                ...
                string jsonResponse = await GeminiApiService.CallGeminiAsync(currentPrompt, config.geminiKey);
