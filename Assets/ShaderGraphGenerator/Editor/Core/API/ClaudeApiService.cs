...
namespace ShaderGraphGenerator.Editor
{
    ...
    public static class ClaudeApiService
    {
        ...
        /// <summary>
        /// Same as CallClaudeAsync but also returns input/output token counts.
        /// </summary>
        public static async Task<(string content, int inputTokens, int outputTokens)> CallClaudeWithUsageAsync(
            string prompt, string apiKey)
        {
            ...
            using (UnityWebRequest www = new UnityWebRequest(URL, "POST"))
            {
                ...
                try { inTok  = (int)(parsed.usage?.input_tokens  ?? 0); } catch { }
                try { outTok = (int)(parsed.usage?.output_tokens ?? 0); } catch { }
                return ((string)parsed.content[0].text, inTok, outTok);
            }
        }
    }
}
