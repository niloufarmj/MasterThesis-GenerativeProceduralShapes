...
void CartoonCar_float(
    ...
    float doorHalfW = max(DoorWidth, 0.001) * 0.5;
    float doorHalfH = max(DoorHeight, 0.001) * 0.5;
    float doorR = clamp(DoorRadius, 0.0, min(doorHalfW, doorHalfH));
    float2 doorPos = float2(CabinOffset.x, BodyOffsetY + BodyHeight * 0.1);
    float dDoorRaw = sdRoundBox(p - doorPos, float2(doorHalfW, doorHalfH), doorR);
...
// ------------------------------------------------------------------------
//  This function produces a **2D cartoon car silhouette** constructed from analytic SDFs. The resulting shape is composed of:
//  - A main rectangular body with smoothly rounded corners.
//  - An upper cabin section with two rounded rectangular windows.
//  - Two large circular wheels placed symmetrically beneath the body, each with a smaller circular rim at the center.
//  - A segmented door on one side, featuring an adjustable width, height, and corner radius, with a small rounded handle.
//  - Additional details include a small round headlight at the front and a taillight at the rear.
//  The overall composition is smooth, with a playful and cartoon-like appearance.
// ------------------------------------------------------------------------
