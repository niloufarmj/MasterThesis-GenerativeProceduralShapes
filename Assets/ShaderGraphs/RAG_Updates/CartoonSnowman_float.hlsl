...
void CartoonSnowman_float(
    ...
    float scarfStripePhase = p.y * ScarfStripeCount;
    float dScarfStripe = abs(frac(scarfStripePhase) - 0.5) - (ScarfStripeThickness * 0.5);
    float scarfStripeAA = max(fwidth(scarfStripePhase), 0.001);
    float scarfStripeFactor = 1.0 - smoothstep(0.0, scarfStripeAA, dScarfStripe);
    float4 finalScarfColor = lerp(ScarfColor, ScarfStripeColor, scarfStripeFactor);

    sm_layer(res, finalScarfColor, 1.0 - smoothstep(0.0, aa, dScarf));
