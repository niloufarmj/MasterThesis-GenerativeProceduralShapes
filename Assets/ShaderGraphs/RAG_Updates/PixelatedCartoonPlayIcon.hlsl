...
void CartoonPlayIcon_float(float2 UV, float Size, float Width, float Height, float Roundness, float OutlineThickness, float4 OutlineColor, float4 ColorTop, float4 ColorBottom, float4 ColorLeft, float4 ColorRight, float PixelCount, out float4 outColor) {
    // Pixelate UV coordinates to create a mosaic/pixel-art effect
    UV = floor(UV * PixelCount) / max(PixelCount, 0.0001);
...
//  Signed Distance Functions (SDFs), modified to display a pixelated/
//  mosaic grid effect.
//
//  The visual result is composed of:
//  - A right-pointing isosceles triangle, commonly associated with "Play"
//    media controls, strictly constrained to a customizable pixel grid.
//  - A distinctive 4-zone coloring scheme where the interior is split 
//    diagonally into Top, Bottom, Left, and Right segments.
//  - Configurable rounded corners that soften the underlying continuous geometry.
//
...
//
//  A thick, solid outline surrounds the shape, rendered to align with the 
//  pixelized UV layout for an authentic retro/mosaic aesthetic.
// ------------------------------------------------------------------------